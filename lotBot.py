...
@dp.callback_query_handler(lambda call: call.data[:12] in ("story_topup_", "story_toadm_"))
    ...
    if await ch().rules_checker(call.from_user.id) == (True, False):
        ...
        if count_lines == 0:
            if call.data[6:11] == "toadm":
                ...
                await call.message.answer("<b>У пользователя нет операций</b>", reply_markup=keyboard,
                                       parse_mode="HTML")
            else:
                keyboard.add(await b().BT_Close())
                await call.message.answer("<b>У вас пока что нет операций</b>", reply_markup=keyboard, parse_mode="HTML")
        else:
            ...
            while N != 0:
                ...
                story_topup += line
                N-=1
            b_next = types.InlineKeyboardButton('\U000027A1', callback_data=f"story_{call.data[6:11]}_{format(current_page + 1, '07')}_{user_id}")
            b_back = types.InlineKeyboardButton('\U00002B05', callback_data=f"story_{call.data[6:11]}_{format(current_page - 1, '07')}_{user_id}")
            ...
            if call.data[6:11] == "toadm":
                keyboard.row(types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"back_adm_info_user_{user_id}"))
                keyboard.row(await b().BT_AdmLk())
            else:
                keyboard.row(await b().BT_Lk())
            await bot.edit_message_text(story_topup, call.from_user.id, call.message.message_id, reply_markup=keyboard, parse_mode="HTML")
    ...
    if await ch().rules_checker(call.from_user.id) == (True, False):
        ...
        await bot.edit_message_text(m_topup_create, call.from_user.id, call.message.message_id,
                                    reply_markup=types.InlineKeyboardMarkup(1).add(
                                        types.InlineKeyboardButton('\U0001F4B8 Оплатить', url=url_link),
                                        types.InlineKeyboardButton('\U00002705 Перевод выполнил',
                                        callback_data=f"check_topup_{way_topup}_{format(get_comm, '7')}")), parse_mode="HTML")
...
#История игр
@dp.callback_query_handler(lambda call: call.data[:12] in ("story_games_", "story_gaadm_"))
        ...
        try:
            ...
            if ((way == "qiwi") and ((len(str(message.text)) != 11) or (message.text[:1] != "7"))) or (way == "bank") and (len(message.text) != 16):
                ...
                keyboard.add(types.InlineKeyboardButton('\U0000267B Ввести заново',
                             ...
                             await b().BT_Lk())
                await message.answer(txt_wrong, reply_markup=keyboard, parse_mode="Markdown")
            else:
                if way == "qiwi":
                    lineway = "QIWI"
                    req = "Номер телефона - *+"
...
async def sticker_reply(message):
    if await ch().rules_checker(message.from_user.id) == (True, False):
        ...
        if message.forward_from:
            await lotBot.uni_reply(message)
        ...
        try:
            ...
            if (int(message.text) > max_sum) or (int(message.text) < min_sum):
                ...
                if (await state.get_data())['type'] == "oper":
                    ...
                    keyboard.add(types.InlineKeyboardButton("\U00002705 Подтвердить", callback_data=f"adm_change_balance_{way}_{format(int(message.text), '06')}_{user_id}"),
                                 types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"chk_change_balance_{user_id}"))
            ...
            keyboard.add(b_enter_again, b_lk)
...
@dp.callback_query_handler(lambda call: call.data[:19] == "adm_change_balance_")
        ...
        if await db.adm_valid_check(call.from_user.id):
            if await db.adm_lvl_check(call.from_user.id) > conf.master_lvl:
                if call.data[19:22] == "nul":
                    ...
                    await bot.edit_message_text(f"<b>Баланс пользователя <code>{call.data[23:]}</code> обнулен</b>", call.from_user.id,
                                                call.message.message_id, reply_markup=types.InlineKeyboardMarkup(1).add(
                            types.InlineKeyboardButton("\U00002B05 Назад",
                                                       callback_data=f"back_adm_info_user_{call.data[23:]}"),
                            await b().BT_AdmLk()), parse_mode="HTML")
                        ...
                        if call.data[19:22] == "add":
                            ...
                            if int(call.data[23:29]) > await db.get_user_balance(call.data[30:]):
                                ...
                                call.from_user.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup(1).add(
                                        types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"chk_change_balance_{call.data[30:]}"),
                                        await b().BT_AdmLk()), parse_mode="HTML")
                            else:
                                await db.set_withdraw_balance(call.data[30:], int(call.data[23:29]))
                                text_act = "уменьшен"
                        await bot.edit_message_text(f"<b>Баланс пользователя <code>{call.data[30:]}</code> {text_act} на {int(call.data[23:29])}₽</b>",
                    ...
                    await bot.edit_message_text(f"<b>Новый баланс пользователя <code>{call.data[30:]}</code> - {int(call.data[23:29])}₽</b>", call.from_user.id,
                                                call.message.message_id, reply_markup=types.InlineKeyboardMarkup(1).add(
                            types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"back_adm_info_user_{call.data[30:]}"),
                            await b().BT_AdmLk()), parse_mode="HTML")
            else:
...
@dp.callback_query_handler(lambda call: call.data[:19] == "chk_change_balance_")
            ...
            if await db.adm_lvl_check(call.from_user.id) > conf.master_lvl:
                if call.data[19:22] not in ("add", "rem", "nul", "set"):
                    keyboard = types.InlineKeyboardMarkup(2).add(
                        types.InlineKeyboardButton("\U0001F4C8 Добавить", callback_data=f"chk_change_balance_add_{call.data[19:]}"),
                        types.InlineKeyboardButton("\U0001F4C9 Снять", callback_data=f"chk_change_balance_rem_{call.data[19:]}"),
                        types.InlineKeyboardButton("\U0001F4A3 Обнулить", callback_data=f"chk_change_balance_nul_{call.data[19:]}"),
                        types.InlineKeyboardButton("\U0001F4DD Изменить", callback_data=f"chk_change_balance_set_{call.data[19:]}"))
                    keyboard.row(types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"back_adm_info_user_{call.data[19:]}"))
                    keyboard.row(await b().BT_AdmLk())
                    await bot.edit_message_text(f"<b>Выберите действие с балансом пользователя <code>{call.data[19:]}</code></b>", call.from_user.id,
                    ...
                    if call.data[19:22] in ("add", "rem"):
                        if call.data[28:29] != "_":
                            if call.data[19:22] == "add":
                                ...
                                text_act = "уменьшения"
                            await bot.edit_message_text(f"<b>Выберите сумму {text_act} баланса пользователя <code>{call.data[23:]}</code></b>",
                                                        call.from_user.id, call.message.message_id, reply_markup=
                                                        await b().KB_Sum("admin", call.data[23:], call.data[19:22]),
                                                        parse_mode="HTML")
                        else:
                            if call.data[19:22] == "add":
                                text_act = "добавления"
                        ...
                        await bot.edit_message_text(f"<b>Вы действительно хотите обнулить баланс пользователя <code>{call.data[23:]}</code>?</b>", call.from_user.id,
                                                    call.message.message_id, reply_markup=types.InlineKeyboardMarkup(1).add(
                                types.InlineKeyboardButton("\U0001F512 Да", callback_data=f"adm_change_balance_nul_{call.data[23:]}"),
                                types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"chk_change_balance_{call.data[23:]}"),
                                await b().BT_AdmLk()), parse_mode="HTML")
                    elif call.data[19:22] == "set":
                        await bot.edit_message_text(
                            f"<b>Введите новый баланс пользователя <code>{call.data[23:]}</code></b>", call.from_user.id,
                            call.message.message_id, parse_mode="HTML")
...
@dp.callback_query_handler(lambda call: call.data[:15] == "chk_block_user_")
    ...
    if await ch().rules_checker(call.from_user.id) == (True, False):
        if await db.adm_valid_check(call.from_user.id):
            if await db.adm_lvl_check(call.from_user.id) > conf.middle_lvl:
                ...
                await bot.edit_message_text(f"<b>Вы действительно хотите {text_act} пользователя <code>{call.data[15:]}</code>?</b>",
                                            call.from_user.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup(1).add(
                        types.InlineKeyboardButton("\U0001F512 Да", callback_data=f"adm_block_user_{call.data[15:]}"),
                        types.InlineKeyboardButton("\U00002B05 Назад", callback_data=f"back_adm_info_user_{call.data[15:]}"),
                        await b().BT_AdmLk()), parse_mode="HTML")
            else:
...
@dp.callback_query_handler(lambda call: call.data[:15] == "adm_block_user_")
        ...
        if await db.adm_valid_check(call.from_user.id):
            if await db.adm_lvl_check(call.from_user.id) > conf.middle_lvl:
                ...
                await bot.edit_message_text(f"<b>Пользователь <code>{call.data[15:]}</code> успешно {text_act}!</b>",
                                            call.from_user.id, call.message.message_id, reply_markup=
                                            types.InlineKeyboardMarkup(1).add(types.InlineKeyboardButton(
                                                "\U00002B05 Назад", callback_data=f"back_adm_info_user_{call.data[15:]}"),
                                                await b().BT_AdmLk()), parse_mode="HTML")
            else:
                await msg().no_access(call.from_user.id, "2 (Master)", call.message.message_id)
        else:
...
@dp.callback_query_handler(lambda call: call.data[:19] == "back_adm_info_user_")
async def user_info(call):
    if await ch().rules_checker(call.from_user.id) == (True, False):
        if await db.adm_valid_check(call.from_user.id):
            await adm_user_info(call.from_user.id, call.data[19:], False, call.message.message_id)
        else:
            logging.warning(f"Not valid admin {call.from_user.id} tried to get access to admin panel with message - {call.from_user.id}")
...
async def adm_user_info(adm_id, user_id, send, msg_id = ""):
    if await ch().rules_checker(adm_id) == (True, False):
        if await db.adm_valid_check(adm_id):
            if user_id.isdigit() and await db.get_user_exists(user_id) or await db.get_user_exists(user_id, "username ILIKE"):
                if user_id.isdigit():
                    key = "user_id ="
                else:
                    key = "username ILIKE"
                id_sys, user_id, join_date, balance, name, lastname, username, rules_acc, ban = [x for x in await db.adm_user_info(user_id, key)]
                join_date = str(join_date)
                keyboard = types.InlineKeyboardMarkup(2)
                keyboard.add(
                    types.InlineKeyboardButton("Операции", callback_data=f"story_toadm_0000001_{user_id}"),
                    types.InlineKeyboardButton("Игры", callback_data=f"story_gaadm_0000001_{user_id}"))
                ...
                if await db.adm_lvl_check(adm_id) > conf.junior_lvl:
                    keyboard.row(types.InlineKeyboardButton("\U00002754 Проверить", callback_data=f"adm_check_user_{user_id}"))
                    if await db.adm_lvl_check(adm_id) > conf.middle_lvl:
                        ...
                        keyboard.row(types.InlineKeyboardButton(text_act, callback_data=f"chk_block_user_{user_id}"))
                        if await db.adm_lvl_check(adm_id) > conf.master_lvl:
                            keyboard.row(types.InlineKeyboardButton("\U0001F4DD	Изменить баланс", callback_data=f"chk_change_balance_{user_id}"))
                keyboard.row(types.InlineKeyboardButton("\U0000267B Другой пользователь",
                ...
                if send:
                    ...
                    await bot.edit_message_text(f"<b>Данные о пользователе:\n\nID - </b><code>{user_id}</code>\n"
                                                   f"<b>Имя -</b> {name}{lastname_info}<b>Баланс -</b> {format(balance, '.0f')}₽\n"
                                                   f"<b>Дата регистрации -</b> {join_date[:10]}\n<b>Правила - {rules_info}\n"
                                                   f"Наличие бана - {ban_info}</b>", adm_id, msg_id, reply_markup=keyboard,
                                                parse_mode="HTML")
            else:
...
async def change_withd_way(call):
    if await ch().rules_checker(call.from_user.id) == (True, False):
        if await db.adm_valid_check(call.from_user.id):
            if await db.adm_lvl_check(call.from_user.id) > conf.middle_lvl:
                logging.warning(f"Admin {call.from_user.id} change requisites from {await db.get_requisites(call.data[22:32].lstrip('0'))} to {call.data[33:]}")
...
async def uni_reply(message):
    if await ch().rules_checker(message.from_user.id) == (True, False):
        if message.forward_from:
            ...
            if await db.get_user_exists(message.forward_from.id) and await db.adm_check(message.from_user.id) and await db.adm_valid_check(message.from_user.id):
                keyboard = types.InlineKeyboardMarkup(1).add(types.InlineKeyboardButton("\U0001F4D6 Информация о пользователе",
            ...
            await message.answer(f"<b>Пользователь запретил доступ к собственной информации настройками приватности</b>",
                                   reply_markup=await b().KB_Menu(),parse_mode="HTML")
