...
class ButtonsTg:
    ...
    async def KB_Sum(self, type, key, way="", from_where=""):
        async def BT_sum(type, key, way, emoji, sum):
            if type == "game":
                button_sum = types.InlineKeyboardButton(text=f"{emoji} {sum} ₽", callback_data=f"bet_{key}_{format(sum, '06')}")
            elif type == "oper":
        ...
        if type == "game":
            if key in ("bowl", "cube", "slot"):
                buttons.append([types.InlineKeyboardButton(text="\U0001F4DD Другая сумма", callback_data=f"{key}_bet_other_sum")])
    ...
    async def ButtonTopupCheck(self, user_id, way_topup, id_pay, msg_id):
        if await db.get_topup_accured(id_pay) == 0:
            ...
            await bot.send_message(user_id, "*Данная транзакция уже выполнена*",
                                   reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[await self.BT_close()]), parse_mode="Markdown")
    ...
    async def KBT_Account(self, user_id):
        ...
        return acc_info, types.InlineKeyboardMarkup(inline_keyboard=[
            [
                types.InlineKeyboardButton(text="\U0001F4B5 Пополнить", callback_data="topup_balance_main_"),
                types.InlineKeyboardButton(text="\U0001F4B8 Вывести", callback_data="withd_balance_main_")
            ],
            [types.InlineKeyboardButton(text="\U0001F4D6 Операции", callback_data="story_topup_0000001_")],
            [types.InlineKeyboardButton(text="\U0001F3B2 История игр", callback_data="story_games_1_")]
        ])
    ...
    async def KBT_Bpmanag(self, user_id):
        ...
        return text, types.InlineKeyboardMarkup(inline_keyboard=[
            [
                types.InlineKeyboardButton(text="Транзакции", callback_data=f"control_transact"),
                types.InlineKeyboardButton(text='Пользователи', callback_data=f"control_users")
            ],
                [types.InlineKeyboardButton(text='Админы', callback_data=f"control_admin")],
        ])
    ...
    async def BT_AdmLk(self, emoji="\U0001F464"):
        return types.InlineKeyboardButton(text=f'{emoji} В главное меню', callback_data="main_bpmanag")
